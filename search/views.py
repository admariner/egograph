...
def result(request, query):
    ...
    # Create graph data
    def create_graph_data(keep_parallel_edges=True):
        ...
        return {
            ...
            'edgelist': [(graph_node_list[e['from']], graph_node_list[e['to']], e['value']) for e in graph_data['edges']] # [(from, to, value)]
        }

    # Rankings / network analysis / debug
    def network_analysis(debug=False):

        # Get edge list - always use graph data so they match
        graph_edgelist = create_graph_data()['edgelist']
    ...
    # Show debug
    network_analysis(debug=settings.DEBUG)

    ########################################################
    # Render

    # Get graph obj
    graph_obj = network_analysis()['graph']['G']

    # Render
    return render(request, 'search/result.html', context = {
        'input_value': clean_query,
        'graph_data': json.dumps(create_graph_data()['graph_data']),
        'suggestions': network_analysis()['rankings'],
        'stats': {
